...
- **Модальное окно бронирования**: в форме создания/редактирования брони отображается блок «Сумма предоплаты: X ₽»
- Парсинг данных: ФИО гостя, email, даты, номер комнаты, стоимость, номер бронирования, количество гостей
- Расчёт предоплаты: (общая сумма / кол. ночей) × 3 (не более общей суммы)
- Описание предоплаты в счёте: «3 суток проживания» (при бронировании 3+ ночей)
...
- **Два QR-кода для оплаты** (ГОСТ Р 56042-2014):
  - QR предоплаты ((общая сумма / ночей) × 3)
...
├── src/
│   ├── content/
│   │   ├── content.js               — MutationObserver, кнопки, инъекция «Сумма предоплаты» в модальное окно
...
│   ├── utils/
│   │   ├── data-parser.js           — Парсер DOM + parseBookingModalData для модального окна бронирования
...
Скидка применяется только к QR-коду полной оплаты. Предоплата ((сумма/ночей)×3) всегда без скидки.
