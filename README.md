...
В комплекте поставляется скрипт `update.ps1` для автоматической загрузки и установки обновлений:

```powershell
# Запуск из папки расширения
.\update.ps1

# Или с указанием параметров
.\update.ps1 -RepoOwner "MakarenD" -RepoName "KonturExpansionChrome" -InstallPath "C:\KonturExpansionChrome"
```

**Важно:** Скрипт устанавливает расширение в `C:\KonturExpansionChrome` — единая папка для всех пользователей.
...
2. Добавьте `печать.png` и `подпись.png` (игнорируются Git, но нужны для работы)
3. Создайте ZIP-архив (например, `KonturExpansionChrome-v1.2.0.zip`)
